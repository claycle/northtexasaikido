...
function buildMessage() {
	...
	if (!isset($_POST['phonecGhvbmUK'])) {throw new Exception("phone not set - check hash agreement cGhvbmUK");}
	if (!isset($_POST['commentY29tbWVudAo='])) {throw new Exception("comment not set - check hash agreement Y29tbWVudAo=");}

	//$pathToMon = "";

	//$message .= "<h2>Inquiry from " . filter_input(INPUT_POST, 'namebmFtZQo=', FILTER_SANITIZE_STRING) . "</h2>\r\n";
	$message .= "<div style='border-radius: 5px; background-color: rgba(89,25,2, 0.1); padding: 1em;'>";
	$message .= filter_input(INPUT_POST, 'commentY29tbWVudAo=', FILTER_SANITIZE_STRING);
	$message .= "</div>\r\n";
	$message .= "<h3>Contact Information</h3>\n";
	$message .= "<p><strong>Name:</strong> " . filter_input(INPUT_POST, 'namebmFtZQo=', FILTER_SANITIZE_STRING) . "<br />\r\n";
	$message .= "<p><strong>Email:</strong> " . filter_input(INPUT_POST, 'emailZW1haWwK', FILTER_SANITIZE_EMAIL) . "<br />\r\n";
	$message .= "<p><strong>Phone:</strong> " . filter_input(INPUT_POST, 'phonecGhvbmUK', FILTER_SANITIZE_STRING) . "<br />\r\n";
	$message .= "<div style='text-align: center; margin-left: auto; margin-right: auto; width: 100%;'>";
...
function sendEmailMessage() {
	...
	$header .= "Reply-to: " . filter_input(INPUT_POST, 'emailZW1haWwK', FILTER_SANITIZE_EMAIL) . "\r\n";
	$header .= "MIME-Version: 1.0\r\n";
	$header .= "Content-type: text/html\r\n";

	$retval = mail($to, $subject, $message, $header);
	if (!$retval) {throw new Exception("Message could not be sent.");}
...
function recordInDatabase() {
	...
	$conn = new mysqli($DBCONFIG_HOST, $DBCONFIG_USER, $DBCONFIG_PWD, $DBCONFIG_DBNAME);
	// Check connection
	...
	$stmt->bind_param("sss", $_POST['namebmFtZQo='], $_POST['emailZW1haWwK'], $_POST['phonecGhvbmUK']);
	$stmt->execute();
	$stmt->close();
	$conn->close();
